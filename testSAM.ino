...
volatile unsigned long pwm_ch1 = 0;  // pulse width input ch1
volatile unsigned long pwm_ch2 = 0;  // pulse width input ch2
volatile unsigned long pwm_ch3 = 0;  // pulse width input ch3
...
void setup()
{
  ...
  pinMode(9, INPUT);        // CH3 input pin D9
  pinMode(10, INPUT);       // CH2 input pin D10
  pinMode(11, INPUT);       // CH1 input D11
  ...
  write_FRAM(0, '%');    // write a byte to begin logging
  addr_count++;
...
void loop()
{
  ...
  if (addr_count < 32000 && start_record)
  {
    write_INT(addr_count, start_msg);
    addr_count += 2;
    
    write_INT(addr_count, int(millis() - start_time));
    addr_count += 2;
    ...
    write_INT(addr_count, int(pwm_ch1));
    addr_count += 2;
    write_INT(addr_count, int(pwm_ch2));
    addr_count += 2;
    write_INT(addr_count, int(pwm_ch3));
    addr_count += 2;
